...
=======================================
This is a Loading Benchmark Service that test REST API and return the benchmark measurements.
This project built based on "https://github.com/jeffbski/bench-rest"

Getting Started
===============
1. Clone the repository
2. From node.js command, go to the repository folder
3. Run “npm i" to install last version of the dependences
...
A couple of key metrics to be aware of:
---
...
- pass flow as just a single REST operation, ex: 
...
passing as array implies no setup/teardown and these are the main operations_

```javascript
var flow =[
{  
  put: '_http://localhost:8000/foo_', json:'mydata'},
  { get:'_http://localhost:8000/foo_' }
];
```
...
- iterations - required number of flow iterations to perform on the main flow (as well as beforeMain and afterMainsetup/teardown operations)
- prealloc - optional max number of iterations to preallocate before starting, defaults to lesser of 100K and iterations. When using large number of iterations or large payload per iteration, it can be necessary to adjustthis for optimal memory use.
...
```javascript
  var flow =* {
    before:[],  _// REST operations to perform before anything starts_
    beforeMain:[],  _// REST operations to perform before each iteration_
    main:[],        _// REST operations to perform for each iteration_
    afterMain:[],   _// REST operations to perform after each iteration_
    after:[]        _// REST operations to perform after everything is finished_
  };
```


Each operation can have the following properties:
One of these common REST properties get, head, put, post, patch, del (using del rather than delete since delete is a JS reserved word) with a value pointing to the URI, ex: 
```
...
```
any other properties/options which are valid for mikeal/request - see pre/post processing - optional array as beforeHooks and afterHooks which can perform processing before and/or after an operation. See Pre/post operation processing section below for details.
...
1. `#{INDEX}` - replaced with the zero based counter/index of the iteration
...
2. `#{INDEX.**}` – replaced by the corresponding dynamic input value The arrays of input data in the: runOptions. dynamicInputArrays. The “**” is the index of the dynamic input array (dynamicInputArrays)
The value taken from the dynamic input array is the Math.MIN(iteration# , dynamicInputArrays.length-1)
For example:
...
- startStepTimer - used in beforeHooks to start a timer for this step named step\_OPIDX where OPIDX is the zero based index of the step in the flow. Be sure to call endStepTimer in afterHooks to end it. Provides detailed stats for an individual step in a flow.
